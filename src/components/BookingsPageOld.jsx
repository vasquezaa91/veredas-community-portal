...
const BookingsPage = ({ windowWidth }) => {
  console.log('BookingsPage rendered with windowWidth:', windowWidth)
  ...
  // Azure Database Integration Functions (prepared for deployment)
  const azureConfig = {
    // These would be environment variables in production
    functionAppUrl: process.env.REACT_APP_AZURE_FUNCTION_URL || 'https://your-function-app.azurewebsites.net',
    apiKey: process.env.REACT_APP_AZURE_API_KEY || '',
    endpoints: {
      getReservations: '/api/reservations',
      createReservation: '/api/reservations',
      updateReservation: '/api/reservations',
      deleteReservation: '/api/reservations'
    }
  }

  // Fetch reservations from Azure database
  const fetchReservations = async (date, facility) => {
    setLoading(true)
    try {
      // In production, this would call Azure Functions
      const response = await fetch(`${azureConfig.functionAppUrl}${azureConfig.endpoints.getReservations}?date=${formatDate(date)}&facility=${facility}`, {
        headers: {
          'X-Functions-Key': azureConfig.apiKey,
  ...
  const createReservation = async (reservationData) => {
    setLoading(true)
    try {
      const response = await fetch(`${azureConfig.functionAppUrl}${azureConfig.endpoints.createReservation}`, {
        method: 'POST',
        headers: {
          'X-Functions-Key': azureConfig.apiKey,
          'Content-Type': 'application/json'
  ...
  return (
    <div style={{ backgroundColor: 'white', borderRadius: '8px', padding: windowWidth < 640 ? '1.5rem' : '2rem', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
      ...
      {/* Debug info */}
      <div style={{ backgroundColor: '#f0f9ff', padding: '1rem', marginBottom: '1rem', borderRadius: '8px', border: '1px solid #0ea5e9' }}>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#0c4a6e' }}>
          DEBUG: Component loaded successfully. Window width: {windowWidth}px
        </p>
      </div>
            ...
            <form onSubmit={handleSubmitBooking}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input
                  type="number"
                  min="1"
                  max={selectedFacilityData?.capacity?.match(/\d+/)?.[0] || 50}
